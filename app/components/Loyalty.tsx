...
const tierMeta = [
  { cashback: "3%", ring: "from-[#b07a4b] to-[#8a5a2f]" },
  { cashback: "5%", ring: "from-[#b9c0c9] to-[#8b939c]" },
  { cashback: "8%", ring: "from-[#e6bf5a] to-[#c8992f]" },
  { cashback: "12%", ring: "from-[#4a5568] to-[#1f2733]" },
];

export function Loyalty({ dict }: { dict: Dictionary["loyalty"] }) {
  return (
    <section
      id="loyalty"
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-lg text-cream/70">{dict.subtitle}</p>
        </div>
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {dict.tiers.map((tier, i) => (
            ...
            >
              <span
                className={`inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br ${tierMeta[i].ring} text-cream shadow-lg`}
              >
              ...
              <p className="text-sm text-cream/60">{dict.cashbackNote}</p>
        ...
        <div className="mt-8 flex flex-col items-center justify-center gap-3 rounded-3xl border border-cream/10 bg-cream/5 p-6 text-center sm:flex-row sm:text-left">
          ...
          <p className="text-cream/85">
            <span className="font-bold text-cream">{dict.referralStrong}</span>{" "}
            {dict.referralRest}
          </p>
        </div>
      </div>
